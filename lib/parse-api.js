...
Module.prototype = {
		...
		this.app.find('Card', {squishleId: doc.guid}, function (err, res) {
		  	if(err) cb(err);
		  		...
		  		if (res.results.length > 0){
					...
					_self.app.insert('Card', {question: doc.title, squishleId: doc.guid}, function(err, data){
						if(err) cb(err);
						else{
	...
	_insertChoice: function(cardId, doc, index, cb){
		...
		if (img === '' && txt === ''){
			...
			this.app.find('Choice', {cardId: cardId}, function (err, res) {
			  	if(err) cb(err);
			  	else{
			  		...
			  		if(found > -1){
			  			...
			  			_self.app.insert('Choice', {cardId: cardId, image: img, text: txt}, function(err, data){
							if(err) cb(err);
							else{
	...
	_insertCardCategory: function(cardId, categoryId, cb){
		var _self = this;
		this.app.find('CardCategory', {cardId: cardId, categoryId: categoryId}, function (err, res) {
		  	if(err) cb(err);
		  	else{
		  		if (res.results.length > 0){
		  			cb(null, res.results[0].objectId);
					...
					_self.app.insert('CardCategory', {cardId: cardId, categoryId: categoryId}, function(err, data){
						if(err) cb(err);
						else{
							cb(null, data.objectId);
						}
